...
# | name | rep | time | OD600 | P | P_ISTD |
# |------|-----|------|-------|---|--------|
# | glucose | 1 | 24 | 0.01 | 50.6 | 10.3 |
# | glucose | 2 | 24 | 0.03 | 48.1 | 12.1 |

# Read data
raw = read_csv(input_path) %>%
  mutate(
    mmol=(P/P_ISTD-coef(std[1])[1])/coef(std[1])[2]*culture_vol
    )
...
# Calculate productivity (mmol/h/g CDW)
rate = raw %>%
  group_by(name, rep) %>%
  arrange(time) %>%
  mutate(
    prod = (mmol - lag(mmol)) / (time - lag(time)) / (OD600 + lag(OD600)) / cell_mass / culture_vol * 2
    )
